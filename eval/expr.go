...
func evalFieldAccess(vm *Vm, e *syntax.FieldAccessExpr) (Value, error) {
	...
	case FuncValue:
		// Element functions can have associated sub-functions in their scope
		// (e.g., table.cell, table.header)
		if t.Func != nil && t.Func.Scope != nil {
			if binding := t.Func.Scope.Get(fieldName); binding != nil {
				return binding.Value, nil
			}
		}
		return nil, &FieldNotFoundError{Field: fieldName, Type: target.Type(), Span: e.ToUntyped().Span()}
