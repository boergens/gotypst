...
func TestSyntaxNodeSynthesize(t *testing.T) {
	...
	span, _ := SpanFromNumber(FileIdFromRaw(1), 100)
...
func TestLinkedNodeFind(t *testing.T) {
	...
	// Assign spans
	root.Numberize(FileIdFromRaw(1), [2]uint64{2, 1000})
	...
	nonexistent, _ := SpanFromNumber(FileIdFromRaw(1), 999999)
...
func TestNumberize(t *testing.T) {
	...
	err := root.Numberize(FileIdFromRaw(1), [2]uint64{2, 1000})
	if err != nil {
	...
	if root.Span().RawFileId() != 1 {
		t.Error("expected file ID 1")
	}
	if child1.Span().RawFileId() != 1 {
		t.Error("expected file ID 1")
	}
}
...
func TestNumberizeEmptyRange(t *testing.T) {
	node := Leaf(Text, "foo")
	err := node.Numberize(FileIdFromRaw(1), [2]uint64{10, 10})
	if err == nil {
